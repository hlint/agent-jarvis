---
name: workspace
description: 工作区是 AI 的代码执行环境和数据存储区，提供完整的文件系统、脚本运行、npm依赖管理能力。简单操作（如读取已知文件、运行已有稳定脚本）可直接调用工具；复杂操作（如编写新脚本、创建目录结构、管理依赖、调试代码）必须先 review 本 skill 了解完整能力、限制和最佳实践。
---
